...
if ($result && $result->num_rows > 0) {
    ...
    echo "<p>Нет данных в таблице rashod.</p>";
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['Save'])) {
        ...
        $stmt = $conn->prepare("INSERT INTO rashod (date, name, kup, izras, ost, prise, itogo) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssssd", $date, $name, $kup, $izras, $ost, $prise, $itogo);
        if ($stmt->execute()) {
            echo "<script>window.location.href = 'rashod.php';</script>";
        } else {
    ...
    if (isset($_POST['Delete'])) {
        $id = (int)($_POST['id'] ?? 0);
        if ($id > 0) {
            $stmt = $conn->prepare("DELETE FROM rashod WHERE id=?");
